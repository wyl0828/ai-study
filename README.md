...
同时提供独立的后端知识训练入口，把 Java / JVM / Spring / MySQL / Redis 知识卡片轻量纳入训练计划。算法错误只触发算法相关诊断，后端知识卡不和算法错因强行绑定。
...
- 独立 `/knowledge` 页面，分类固定为 Java / JVM / Spring / MySQL / Redis
- 知识卡片包含问题、答案、追问、记忆点、难度、标签和来源链接
- Java 基础、集合、并发只作为 tags 展示，不拆成独立一级 tab
- 第一版使用 MySQL 结构化种子数据，不做 RAG，不根据算法错因强行推荐 MySQL / Redis / Spring
...
| [KNOWLEDGE_TRAINING_DESIGN.md](docs/KNOWLEDGE_TRAINING_DESIGN.md) | 后端知识训练模块设计 |
