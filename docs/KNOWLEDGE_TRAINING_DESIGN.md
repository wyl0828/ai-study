...
- 卡片未展开时展示难度、分类、tags、标题、问题描述和“查看解析，或在此之前使用 AI 自测”。
- 展开后默认只展示题目信息、AI 自测输入框、提交自测按钮和“跳过自测，直接查看解析”。
- 提交自测后展示 AI 点评、评分、命中核心记忆点、标杆回答解析、核心记忆要点、面试官高频追问和“标记已掌握”。
- 点击“跳过自测，直接查看解析”时只显示解析区，不展示虚假的 AI 评分。
...
`answerKeywords` 用于本地模拟 AI 自测评分。它不代表最终 AI Prompt，也不落库。
...
- 自测记录和掌握状态如需持久化，应新增清晰的数据表和 REST 接口。
...
- 点击知识点可以展开和收起。
- 展开后默认不直接展示答案区。
- 点击“提交自测”后出现评分、AI 点评、命中记忆点和解析区。
- 点击“跳过自测，直接查看解析”后显示解析区，但不显示 AI 评分。
