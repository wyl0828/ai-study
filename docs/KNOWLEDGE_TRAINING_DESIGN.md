...
- “标记已掌握”当前主要用于页面内轻量状态；真正的持久化学习连续性来自自测提交写入 `self_test_record`，并由后端更新 `user_knowledge_card_mastery`。
- 当前知识卡内容质量整改已完成：问题改为直白短问法，答案围绕定义、机制、边界和常见坑，不再使用统一扩写模板凑字数。

本次明确不做：

- 不新增独立 RAG 聊天、知识收藏或公开检索相关数据库表。
- 不新增独立 RAG 聊天、知识收藏或公开检索 REST 接口。
...
- 页面加载时并发请求分类和卡片列表。
- 当前列表接口已返回训练页所需的答案、核心记忆点和高频追问；详情接口仍保留用于单卡按需读取和后续收窄列表字段。
- 接口失败或后端未启动时回退本地示例数据，并显示浅提示。
- 自测评分继续在前端本地模拟，不调用真实 AI。
- 自测记录和知识卡掌握度通过 `self_test_record`、`user_knowledge_card_mastery` 和用户学习 REST 接口持久化；页面内“标记已掌握”仍是轻量即时反馈，不作为 durable mastery 的唯一来源。
