...
- **Redis 题目热点缓存**：已接入题目列表、题目详情和题目模板缓存，key 使用 `coach:problem:*:v1` 前缀，TTL 可通过 `PROBLEM_CACHE_*` 环境变量调整；Redis 读写失败只记录 warning 并降级 MySQL，不缓存提交、训练计划、RAG 用户记忆、模拟面试或 Agent SSE 状态。
...
- **Dashboard 真实数据接入**：学习中心已从 mock 数据切到后端真实接口，并从“数据看板”重排为“下一步学习指挥台”：统计卡片之后优先展示今日训练项和按天分页的完整训练计划，再展示薄弱排行、错误类型分布、最近提交、最近模拟面试、模拟面试趋势、合并错题卡和确定性 AI 教练建议；训练计划条目可完成/跳过，也支持手动重新生成。
...
- **RAG 内部检索层**：已新增 `rag_document` / `rag_chunk` MySQL 表、`RagService` 和 `RagRetrieveTool`，在 `OBSERVATION` 后检索题目、知识卡、AI 诊断和当前用户错题记忆；默认 MySQL-only，可选通过 `RAG_VECTOR_ENABLED=true` 启用 MySQL + Qdrant 混合检索。embedding 或 Qdrant 失败会降级为 MySQL-only，不阻塞核心闭环。
...
整体进度处于 **Phase 5 产品打磨收尾阶段**，并完成了知识训练页 V1、知识训练页最终前端打磨、4.1 小范围产品增强、4.2 稳定核心闭环回归护栏、RAG 内部检索层和可选 Qdrant 混合检索切片。SSE 流式诊断已接入前端，Agent 步骤实时展示；完整 demo 复盘已有 `scripts/e2e_demo_smoke.ps1` 和 `docs/FINAL_ACCEPTANCE_CHECKLIST.md` 作为演示前验收入口，截图录屏和面试 Q&A 仍归入最终阶段。
...
- **本地依赖较多**：MySQL、Piston、后端、前端都要启动；Redis 已用于题目热点缓存，但不是核心事实源，Redis 不可用时题目接口会降级 MySQL。
...
- Dashboard 数据继续收口：
  ...
  - 模拟面试趋势来自 `GET /api/users/{userId}/mock-interviews/trends`，按知识卡展示最近得分、上次得分、变化方向和最近卡点。
...
- `frontend/lib/core-loop-stability.node-test.cjs` 覆盖右侧 tab 边界、左侧题目预设提示、SSE fallback 条件、旧流拦截、abort 行为、Dashboard 不导入 mock、最近模拟面试、模拟面试趋势、训练计划跳转、今日优先训练前置和空状态文案。
...
- Redis 热点缓存已经接入题目只读热点数据；后续不应把用户训练状态或 Agent 运行状态放入 Redis 作为唯一存储。
...
9. RAG 内部检索层、可选 Qdrant 混合检索切片和 `RAG_RETRIEVAL` Agent 步骤 ✓
