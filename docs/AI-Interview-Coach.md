...
薄弱点用于生成训练计划和错题复习卡片。这里的 Memory 不是把所有聊天记录塞进模型，而是把长期训练状态结构化存到 MySQL，必要时再由 Agent 读取。当前失败提交会写入 `user_weakness_event`，知识卡低分自测会作为 `SELF_TEST` 来源进入弱点事件，模拟面试低分会作为 `MOCK_INTERVIEW` 来源进入弱点事件，训练项完成会作为 `TRAINING_PLAN_COMPLETED` 写入负向 delta，用于表达“最近改善”。
...
每个 turn 分为 `MAIN` 或 `FOLLOW_UP`，保存题目、用户回答、评分、命中要点、缺失要点、表达问题和 AI 原始 JSON。低分或缺失点写入 `user_weakness_event`，`sourceType=MOCK_INTERVIEW`，用于 Dashboard 和后续训练计划。用户明显卡住时会走本地兜底评价，把追问收窄到最小可回答点；报告生成后会把推荐知识卡写入一份模拟面试复盘训练计划。前端通过 `/mock-interview?sessionId=...` 恢复已报告会话，并在报告区展示平均分、强弱项、推荐知识卡和逐轮问答复盘。该入口不做自由聊天、不上传文档、不输出完整 Java AC 代码。
...
第三优先级当前不纳入 MVP 但保留设计入口：单独 hint 查询、单独 accepted-code review REST 接口、知识卡收藏、通用 RAG 聊天 / 公开 raw RAG 检索 REST 接口、Redis 热点缓存真正接入。当前已新增的 `/rag-chat` 是受控学习资料问答入口，不属于通用聊天或 raw retrieval API。
...
```text
使用 Spring Boot 设计题目、提交、诊断、提示、训练计划、知识卡片和 RAG 检索等核心模块；基于 MyBatis-Plus 接入 MySQL 8，完成题目、提交记录、AI 诊断、薄弱点、训练计划、后端知识卡片和 RAG chunk 等数据持久化；封装 Piston 代码执行服务，支持 Java 代码编译运行、测试用例判定和错误日志解析；接入 Redis 题目列表、题目详情和模板热点缓存，并在 Redis 异常时降级 MySQL。
```
...
- 当前 Redis 已接入题目热点缓存，包括题目列表、题目详情和题目模板。
...
- 用户训练状态、提交记录、AI 诊断、错题卡、训练计划、RAG 用户记忆、模拟面试会话和 Agent SSE 状态仍以 MySQL / 当前请求上下文为准，不放入 Redis 作为唯一存储。
