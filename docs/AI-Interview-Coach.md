...
- RAG V1 作为 Agent 内部 Tool 使用 MySQL 结构化检索；另有 `/rag-chat` 受控学习资料问答入口，但不做通用聊天、联网搜索、上传文档或向量数据库
...
RAG V1 不是通用知识库聊天。它首先是 Agent Workflow 中 `OBSERVATION` 之后的可选 Tool：

```text
Observation -> RagRetrieveTool -> ErrorClassifierTool / CodeReviewTool
```

当前另新增 `/rag-chat` 知识库问答 / 学习资料问答 V1，作为受控学习资料问答入口。它只回答题目、知识卡、历史诊断、错题卡和当前用户学习记录相关问题，复用 MySQL RAG V1 检索能力和现有学习记忆数据，不接入通用联网搜索，不上传文档，不生成完整 AC 代码，不替代代码提交诊断主流程。
...
每个 turn 分为 `MAIN` 或 `FOLLOW_UP`，保存题目、用户回答、评分、命中要点、缺失要点、表达问题和 AI 原始 JSON。低分或缺失点写入 `user_weakness_event`，`sourceType=MOCK_INTERVIEW`，用于 Dashboard 和后续训练计划。用户明显卡住时会走本地兜底评价，把追问收窄到最小可回答点；报告生成后会把推荐知识卡写入一份模拟面试复盘训练计划。该入口不做自由聊天、不上传文档、不输出完整 Java AC 代码。
...
- `/rag-chat` 知识库问答 / 学习资料问答 V1：只回答题目、知识卡、历史诊断、错题卡和当前用户学习记录相关问题；复用 MySQL RAG V1 检索能力和现有学习记忆数据，不接入通用联网搜索，不上传文档，不生成完整 AC 代码，不替代代码提交诊断主流程
...
Dashboard 当前通过以上接口读取真实 MySQL 学习数据，包括统计、薄弱点、弱点事件、错题、最新训练计划、错误类型分布、最近提交和最近模拟面试。前端展示层已把最新训练计划拆成“今日优先训练”和“完整训练计划”：今日任务优先选择 `PENDING` 条目，再选择 `NEEDS_REVIEW` / `RETRY`，全部完成时提示复盘最近错题；完整计划按第 1/2/3 天分页展示。最近模拟面试展示会话状态、平均分和弱点标签，并跳转到 `/mock-interview?sessionId=...` 继续或查看报告。错题卡复用后端 `repeatCount` 和前端同类聚合展示“出现 N 次”、本质问题、修复动作和复盘口令；AI 教练建议由最高薄弱点、今日训练项和最近错题确定性生成，不调用 AI。训练计划条目状态和手动重新生成也已暴露；知识训练页通过用户学习接口保存自测记录。
...
- 已完成：知识库问答 V1，新增 `/rag-chat` 和 `POST /api/rag/chat` 作为受控学习资料问答入口；它只回答题目、知识卡、历史诊断、错题卡和当前用户学习记录相关问题，复用 MySQL RAG V1 与现有学习记忆数据，不接入联网搜索、不上传文档、不生成完整 AC 代码、不替代代码提交诊断主流程
