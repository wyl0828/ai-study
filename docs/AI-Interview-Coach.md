...
- 做题页模板加载已统一为浏览器端请求 `/api/problems/{id}/template`：当前 Hot100 精选 20 题全部显示 LeetCode 风格 `class Solution`，旧 `101-108` 前端兜底已移除。
- Dashboard 已通过用户学习查询接口接入真实 MySQL 数据，并重排为学习中心：顶部统计后优先展示“今日优先训练”和完整训练计划，再展示薄弱知识点排行、错误类型分布、最近提交、最近模拟面试、合并错题卡、后端知识训练入口和确定性 AI 教练建议。
...
- RAG 作为 Agent 内部 Tool 默认使用 MySQL 结构化检索；可选开启 MySQL + Qdrant 混合检索，但 `/rag-chat` 仍只是受控学习资料问答入口，不做通用聊天、联网搜索、上传文档或公开 raw retrieval
- `/mock-interview` 是知识卡升级后的模拟面试入口：AI 主动提问、追问、评分，沉淀用户八股表达能力记录，并把低分或缺失点写入学习记忆
...
失败提交 -> WeaknessTrackerTool -> TrainingPlannerTool
  ↓
MySQL / Redis 热点缓存 / SSE / Agent Trace
  ↓
...
- MVP 当前内置 Hot100 精选 20 道题，优先保证演示闭环
- 暂不追求完整 Hot100 或大规模题库扩展，避免稀释 Agent 面试训练主线
- 题型覆盖数组、哈希表、链表、二叉树、动态规划、贪心、滑动窗口、双指针、栈、二分和网格搜索
...
| 类型 | 当前数量 | 代表题目 |
| --- | ---: | --- |
| 数组 / 贪心 / 区间 | 4 | 两数之和、最大子数组和、合并区间、买卖股票的最佳时机 |
| 哈希表 / 滑动窗口 | 3 | 字母异位词分组、最长连续序列、无重复字符的最长子串 |
| 链表 | 3 | 反转链表、合并两个有序链表、环形链表 |
| 二叉树 | 4 | 中序遍历、层序遍历、最大深度、翻转二叉树 |
| 动态规划 | 2 | 爬楼梯、打家劫舍 |
| 双指针 / 栈 / 二分 / 图搜索 | 4 | 三数之和、有效的括号、二分查找、岛屿数量 |
...
- 当前 Hot100 精选 20 题全部为 Solution 模式。后端通过 `CodeWrapper` 注册表按 `problem.id` 选择 adapter，在调用 Piston 前生成完整 `Main.java`；数据库 `submission.code` 仍保存用户原始 Solution 代码，便于 AI 诊断围绕用户代码而不是包装代码。
...
| 3 | 无重复字符的最长子串 | `public int lengthOfLongestSubstring(String s)` |
| 15 | 三数之和 | `public List<List<Integer>> threeSum(int[] nums)` |
| 20 | 有效的括号 | `public boolean isValid(String s)` |
| 49 | 字母异位词分组 | `public List<List<String>> groupAnagrams(String[] strs)` |
| 53 | 最大子数组和 | `public int maxSubArray(int[] nums)` |
| 56 | 合并区间 | `public int[][] merge(int[][] intervals)` |
...
| 94 | 二叉树的中序遍历 | `public List<Integer> inorderTraversal(TreeNode root)` |
...
| 200 | 岛屿数量 | `public int numIslands(char[][] grid)` |
| 121 | 买卖股票的最佳时机 | `public int maxProfit(int[] prices)` |
| 704 | 二分查找 | `public int search(int[] nums, int target)` |
...
薄弱点用于生成训练计划和错题复习卡片。这里的 Memory 不是把所有聊天记录塞进模型，而是把长期训练状态结构化存到 MySQL，必要时再由 Agent 读取。当前失败提交会写入 `user_weakness_event`，知识卡低分自测会作为 `SELF_TEST` 来源进入弱点事件，模拟面试低分会作为 `MOCK_INTERVIEW` 来源进入弱点事件，训练项首次完成会作为 `TRAINING_PLAN_COMPLETED` 写入负向 delta，用于表达“最近改善”；重复完成同一训练项不会重复降低薄弱点分数。

### 4.8 RAG 检索模块

RAG 不是通用知识库聊天。它首先是 Agent Workflow 中 `OBSERVATION` 之后的可选 Tool：

```text
Observation -> RagRetrieveTool -> ErrorClassifierTool / CodeReviewTool
```

当前另新增 `/rag-chat` 知识库问答 / 学习资料问答 V1，作为受控学习资料问答入口。它只回答题目、知识卡、历史诊断、错题卡和当前用户学习记录相关问题，复用内部 RAG 检索能力和现有学习记忆数据，不接入通用联网搜索，不上传文档，不生成完整 AC 代码，不替代代码提交诊断主流程。完整答案代码请求会被拒绝并改为思路 / 伪代码 / 易错点；粘贴未提交代码要求诊断会被引导回提交判题后的 Agent 诊断链路。

默认实现使用 MySQL 结构化检索；当 `RAG_VECTOR_ENABLED=true` 时，可选启用 MySQL + Qdrant 混合检索。MySQL 仍是 `rag_document` / `rag_chunk` 的事实源，Qdrant 只保存 `rag_chunk.id` 对应的向量点和 payload，embedding 使用独立 OpenAI-compatible `/v1/embeddings` 配置，不复用 Anthropic-compatible chat 诊断接口。检索来源包括：
...
检索规则默认是确定性打分：同用户、同题目、同知识点、同错误类型和关键词命中会提高排序；开启向量后会把规则分和向量相似度融合，默认 `finalScore = round(mysqlRuleScore * 0.40 + vectorSimilarity * 100 * 0.60)`。用户历史记忆必须通过 MySQL `user_id` 和 Qdrant payload filter 双层隔离，避免不同用户错题互相泄漏。RAG 失败只记录 `RAG_RETRIEVAL` failed step 或 warning，不影响代码执行、错误诊断、AC 点评、弱点记录或训练计划。

### 4.9 模拟面试模块

`/mock-interview` 将知识卡从“看题和自测”升级为“面试会话”。后端新增 `mock_interview_session`、`mock_interview_turn` 和 `mock_interview_report`，并用显式状态机管理：
...
每个 turn 分为 `MAIN` 或 `FOLLOW_UP`，保存题目、用户回答、评分、命中要点、缺失要点、表达问题和 AI 原始 JSON。低分或缺失点写入 `user_weakness_event`，`sourceType=MOCK_INTERVIEW`，用于 Dashboard 和后续训练计划。用户明显卡住时会走本地兜底评价，把追问收窄到最小可回答点；报告生成后只会把低分、缺失要点或表达问题对应的推荐知识卡写入一份模拟面试复盘训练计划；强回答且无缺口的报告不强行生成推荐卡或训练计划，可直接进入同类面试复测。前端通过 `/mock-interview?sessionId=...` 恢复已报告会话，并在报告区展示平均分、强弱项、推荐知识卡和逐轮问答复盘。该入口不做自由聊天、不上传文档、不输出完整 Java AC 代码。

### 4.10 训练计划模块

`TrainingPlannerTool` 根据用户薄弱点生成 3 天训练计划。Agent 诊断后的自动计划保留 3 条算法复盘任务，并最多选取 3 张 RAG 命中的知识卡片；如果没有可用的 RAG 知识卡证据，则 fallback 到通用高优先级知识卡。手动重新生成计划仍使用确定性规则，会按 3 天组织为每天 1 个算法复盘任务和 1 个知识卡复习任务。
...
```text
第 1 天：HashMap 使用逻辑
- 算法复盘：两数之和 / HashMap 查询顺序
- 知识卡复习：HashMap 底层结构或等价高优先级知识卡

第 2 天：链表指针操作
- 算法复盘：反转链表 / 返回新头节点
- 知识卡复习：Java 集合或并发基础卡片

第 3 天：状态维护与复盘表达
- 算法复盘：买卖股票的最佳时机 / 最低价状态维护
- 知识卡复习：Spring / MySQL / AI 工程中的高优先级卡片
```
...
- AI 工程包含 Agent、RAG、LangChain 三个前端专题入口，并提供少量本地示例卡作为接口不可用时的兜底训练内容；知识库问答 V1 另由 `/rag-chat` 承载，是受控学习资料问答入口，不是通用 RAG 聊天
...
- `/rag-chat` 知识库问答 / 学习资料问答 V1：只回答题目、知识卡、历史诊断、错题卡和当前用户学习记录相关问题；复用内部 RAG 检索能力和现有学习记忆数据，不接入通用联网搜索，不上传文档，不生成完整 AC 代码，不替代代码提交诊断主流程

暂不做：

- 通用 RAG 聊天 / 公开 raw RAG 检索页面
- AI 模拟追问
- 后续可接入真实 AI 自测调用
- 知识卡收藏和浏览记录
...
`item_type = PROBLEM` 兼容旧算法题训练项；`item_type = KNOWLEDGE_CARD` 表示后端知识卡片复习项。`source_type/source_id/source_summary` 用于解释训练项来源，`status_updated_at` 用于 Dashboard 展示最近完成或跳过的训练项。
...
向量 RAG 字段为可选增强：`vector_point_id` 使用 `rag_chunk.id`，`embedding_model` / `embedding_dim` 记录生成向量的模型和维度，`vector_status` 记录 `INDEXED` 或 `FAILED`。`RAG_VECTOR_ENABLED=false` 时这些字段可以为空，系统行为保持 MySQL-only。

`RagService.checkHealth()` 是内部维护能力，不暴露 raw retrieval：它检查 RAG 表是否可用、系统索引是否为空、系统文档是否重复、知识卡更新后是否存在过期 `KNOWLEDGE_CARD` 文档，并返回系统 chunk / 用户记忆 chunk 数量和 warning 列表。表缺失时健康检查和检索都应降级为空结果或 unhealthy 状态，不阻塞 Agent 主流程。
...
当前 Hot100 精选 20 题统一提交非 `public` 的 `class Solution`。前端不传 `code_mode`，后端只在送入 Piston 前按题号包装测试 harness。
...
- `/knowledge` 页面使用 MySQL 结构化数据，不作为开放 RAG 查询入口；这些知识卡会被内部 RAG 索引，供 Agent 诊断和训练计划内部使用。
...
GET /api/users/{userId}/training-plans/history
GET /api/users/{userId}/training-plans/activities/recent
...
GET /api/users/{userId}/mock-interviews/recent
GET /api/users/{userId}/mock-interviews/trends
POST /api/users/{userId}/knowledge/cards/{cardId}/self-tests
GET /api/users/{userId}/knowledge/cards/{cardId}/self-tests/recent
```

Dashboard 当前通过以上接口读取真实 MySQL 学习数据，包括统计、薄弱点、弱点事件、错题、最新训练计划、训练计划历史、最近训练完成情况、错误类型分布、最近提交、最近模拟面试和同知识点面试趋势。前端展示层已把最新训练计划拆成“今日优先训练”和“完整训练计划”：今日任务优先选择 `PENDING` 条目，再选择 `NEEDS_REVIEW` / `RETRY`，全部完成时提示复盘最近错题；完整计划按第 1/2/3 天分页展示，历史摘要展示旧计划状态、完成数和跳过数，最近训练完成情况展示已完成 / 已跳过条目和推荐来源。最近模拟面试展示会话状态、平均分和弱点标签，并跳转到 `/mock-interview?sessionId=...` 继续或查看报告；模拟面试趋势按知识卡聚合同一知识点多场面试的平均分变化，并展示最近缺失点或表达问题。错题卡复用后端 `repeatCount` 和前端同类聚合展示“出现 N 次”、本质问题、修复动作和复盘口令；AI 教练建议由最高薄弱点、今日训练项和最近错题确定性生成，不调用 AI。训练计划条目状态和手动重新生成也已暴露；知识训练页通过用户学习接口保存自测记录。
...
`RagChunkHit` 保留 `sourceType`、`sourceId`、`score` 和 `matchReason`，用于排查为什么某条 problem / knowledge_card / user memory chunk 被召回。`matchReason` 只做内部解释，不改变“RAG 证据只能辅助 AI 诊断和 AC 点评”的边界。
...
- 已完成：Dashboard 教练化重排，今日优先训练前置，完整训练计划按第 1/2/3 天分页展示，错误统计去除重复薄弱点展示，错题卡按同类错误模式聚合表达，AI 教练建议基于现有学习数据确定性生成
...
- 已完成：训练计划支持 `PROBLEM` / `KNOWLEDGE_CARD` 两类 item，Agent 自动计划最多选取 3 张 RAG 命中的知识卡片，手动重新生成按每天 1 个算法复盘任务和 1 个知识卡复习任务组织
- 已完成：RAG 内部检索层，默认使用 MySQL 结构化检索题目、知识卡、AI 诊断和用户错题卡，并可选启用 MySQL + Qdrant 混合检索；`RAG_RETRIEVAL` Agent Step 进入 SSE 时间线且失败不阻塞主流程
- 已完成：知识库问答 V1，新增 `/rag-chat` 和 `POST /api/rag/chat` 作为受控学习资料问答入口；它只回答题目、知识卡、历史诊断、错题卡和当前用户学习记录相关问题，复用内部 RAG 与现有学习记忆数据，不接入联网搜索、不上传文档、不生成完整 AC 代码、不替代代码提交诊断主流程
- 已完成：模拟面试 V1，新增 `/mock-interview` 和 `/api/mock-interviews`，基于知识卡进行主问题、追问、结构化评分、弱点事件记录和面试报告生成，作为比普通问答更贴近 AI Interview Coach 主线的入口
- 已完成：Redis 只读热点缓存，缓存题目列表、题目详情、题目模板、知识卡分类、知识卡列表和知识卡详情，Redis 异常时降级 MySQL；提交、诊断、知识卡自测/掌握度、训练计划、RAG 用户记忆、模拟面试和 Agent SSE 状态仍不放入 Redis 作为唯一事实源
...
- 完整 LeetCode 题库
- 本地 Docker 沙箱
- 复杂知识掌握度模型和生产级向量 RAG 运维；当前 Qdrant 混合检索为可选本地增强，默认关闭
- 单独 hint 查询接口
- 单独 accepted-code review REST 接口
- 知识卡收藏
- 通用 RAG 聊天 / 公开 raw RAG 检索 REST 接口
- Redis 热点缓存已接入题目和知识卡只读热点数据
- 视频或语音面试
- 多 Agent 协作
- 企业级权限系统
- 复杂 UI 动画
...
第三优先级当前不纳入 MVP 但保留设计入口：单独 hint 查询、单独 accepted-code review REST 接口、知识卡收藏、通用 RAG 聊天 / 公开 raw RAG 检索 REST 接口。Redis 热点缓存已接入题目和知识卡只读热点数据，但用户训练状态、知识卡自测/掌握度、Agent 运行状态和 RAG 用户记忆仍不作为 Redis 唯一存储。当前已新增的 `/rag-chat` 是受控学习资料问答入口，不属于通用聊天或 raw retrieval API。
...
```text
使用 Spring Boot 设计题目、提交、诊断、提示、训练计划、知识卡片和 RAG 检索等核心模块；基于 MyBatis-Plus 接入 MySQL 8，完成题目、提交记录、AI 诊断、薄弱点、训练计划、后端知识卡片和 RAG chunk 等数据持久化；封装 Piston 代码执行服务，支持 Java 代码编译运行、测试用例判定和错误日志解析；接入 Redis 题目列表、题目详情、模板以及知识卡只读热点缓存，并在 Redis 异常时降级 MySQL。
```
...
- 根据薄弱知识点匹配相关题目。
- 生成 3 天训练计划。
- 计划 item 分为 `PROBLEM` 和 `KNOWLEDGE_CARD`，Agent 自动计划最多选取 3 张 RAG 命中的知识卡片；手动重新生成计划按 3 天组织为每天 1 个算法复盘任务和 1 个知识卡复习任务。
- 算法错误只推荐算法相关训练，不把 Two Sum 之类的错误强行映射到 MySQL / Redis / Spring。

### 10.6 Redis 用在哪里？

- 当前 Redis 已接入题目和知识卡热点缓存，包括题目列表、题目详情、题目模板、知识卡分类、知识卡列表和知识卡详情。
- 缓存 key 使用 `coach:problem:list:v1`、`coach:problem:detail:v1:{problemId}`、`coach:problem:template:v1:{problemId}`。
- Redis 是加速层，不是事实源；Redis 读写异常时只记录 warning，接口降级查询 MySQL。
- 用户训练状态、提交记录、AI 诊断、错题卡、知识卡自测/掌握度、训练计划、RAG 用户记忆、模拟面试会话和 Agent SSE 状态仍以 MySQL / 当前请求上下文为准，不放入 Redis 作为唯一存储。
