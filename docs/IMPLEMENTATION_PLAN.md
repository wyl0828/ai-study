...
- `UserController` 已暴露 Dashboard 查询接口：统计、薄弱点、弱点事件、错题、最新训练计划、训练计划状态更新、手动重新生成、最近提交、错误统计。
...
- 暂不把收藏、RAG 检索或 `knowledge_weakness` 表落到后端；知识卡掌握度和自测记录已持久化。
...
状态：Dashboard 读取接口已通过 `UserController` 暴露；手动重新生成训练计划和训练项状态更新已实现。
...
- 确认旧 `101-108` 前端静态 fallback 和 ACM `public class Main` 模板路径不再出现。
