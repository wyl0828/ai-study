...
- 已创建 Spring Boot 后端、MySQL/Redis/CORS 配置、基础实体、Mapper 和统一响应结构。
...
- AC 提交路径：重新判题 -> `CodeReviewTool` 生成轻量代码点评，跳过错题卡和训练计划写入。
...
1. 优化知识训练反馈：
   - 给 AI 点评区补“缺失要点”，根据核心记忆点展示用户还没覆盖的内容。
...
- AC 提交通过后能展示清晰的代码点评；失败提交仍展示错误诊断和 Agent 步骤。
...
- AC 提交后，确认 AI 诊断页能展示轻量代码点评而不是错题诊断。
...
```text
封装 Piston 代码执行服务并作为 `CodeExecutionTool` 接入 Agent Workflow，支持 Java 编译运行、测试用例判定和错误日志解析；基于 MyBatis-Plus 接入 MySQL 8，持久化题目、提交、诊断、训练计划和后端知识卡片；使用 Redis 缓存热门题目与用户训练状态，并通过 SSE 实现 Agent 执行步骤流式返回。
```
