...
项目不是普通刷题平台，也不是 Spring Boot 调大模型 API 的聊天壳，而是一个能够调用判题工具、观察测试结果、分析错误思维、分层提示、记录薄弱点、生成训练计划的面试训练 Agent。当前已在不削弱 Agent 主线的前提下，新增独立后端知识训练入口，把 Java / JVM / Spring / MySQL / Redis 知识卡片轻量纳入统一训练计划。
...
- Dashboard 已通过 `UserController` 接入真实 MySQL 学习数据，展示统计、薄弱点、错题卡、错误统计、最近提交、最新训练计划和后端知识训练入口；单独 hint 查询、独立 accepted-code review REST 端点和手动重新生成训练计划留到后续阶段。
...
- 当前 MVP 种子数据为 8 道题、24 条测试用例、1 个 demo 用户。
- `SubmissionServiceImpl` 保存用户原始代码；Solution 模式题目仅在送入 Piston 前通过 `CodeWrapper.wrap(...)` 包装为 `Main.java`。
...
- 当前 Tool 包括 `CodeExecutionTool`、`ErrorClassifierTool`、`CodeReviewTool`、`WeaknessTrackerTool`、`TrainingPlannerTool`。
- 失败提交路径：重新判题 -> 错误分类 -> 弱点/错题卡持久化 -> 确定性 3 天训练计划。
- AC 提交路径：重新判题 -> `CodeReviewTool` 生成轻量代码点评，跳过错题卡和训练计划写入；代码点评失败时记录失败 step 并降级返回 accepted 结果。
...
- `UserController` 已暴露 Dashboard 查询接口：统计、薄弱点、错题、最新训练计划、最近提交、错误统计。
- Dashboard 从 MySQL 持久化数据读取学习数据，无数据时显示空状态，不回退 mock 数据。
- 当前仍不暴露手动重新生成训练计划和单独 hint 查询接口。
...
状态：进行中。SSE 前端接入、题目预设提示迁移后端、ProblemNavigator 上下题切换、README、知识训练页 V1、4.1 小范围产品增强和 4.2 核心闭环回归护栏已完成；完整 demo 复盘、截图录屏和面试 Q&A 留到最终阶段。
...
1. 优化知识训练反馈：
   ...
   - 低分反馈更像面试官评价，明确指出回答过短、缺少机制、触发条件或优化目的。
   - 暂不做掌握记录、自测记录或知识弱点持久化。
2. 完善 AC 代码点评展示：
...
- AC 点评生成中与失败诊断一样展示 `AgentTimeline`，实时呈现 `PLANNING -> CODE_EXECUTION -> OBSERVATION -> CODE_REVIEW -> COMPLETED`。
...
- `frontend/lib/core-loop-stability.node-test.cjs` 覆盖 tab 边界、预设提示边界、SSE fallback 条件、旧流拦截、abort 行为、Dashboard no mock 和空状态文案。
...
最终阶段再集中考虑：跑通 `101/103/104` 三题完整 demo 并记录失败点、对齐 API/实现/设计文档细节、明确 `hint_record` 最终策略、采集截图或录屏、整理 Agent Workflow 面试 Q&A。
...
- 打开 Dashboard，确认统计、弱点、错误类型分布、错题卡、最近提交和训练计划来自真实查询接口。
- 打开 `/knowledge`，确认知识训练页能加载，分类只显示全部分类 / Java / MySQL / Redis / Spring / JVM。
- 确认搜索、难度筛选、分类筛选有效。
- 展开知识点后默认只显示模拟自测输入框、提交按钮和“跳过自测，直接查看解析”。
- 提交自测后确认出现点评反馈/评分、命中记忆点、标杆回答解析、核心记忆要点、面试官高频追问和标记已掌握。
...
| 题库太多拖慢进度 | MVP 当前稳定 8 道题，不急于扩到 20-30 道 |
...
| 知识训练扩展喧宾夺主 | 知识卡片保持独立入口，训练计划最多混入 1-2 条，不让算法诊断强行推荐八股 |
...
11. 讲解后端设计：Piston 封装、Agent Tool、Observation、Memory、SSE、MySQL，以及 Redis 预留配置和后续热点缓存扩展点。
...
```text
基于 Spring Boot 3、Next.js 14 和 LLM 构建 Java 代码诊断与后端面试训练系统，支持 Java 算法题在线提交、Piston 代码执行、Agent Workflow 编排、测试结果 Observation、错误归因、AC 代码点评、题目预设分层提示、后端知识卡片训练、薄弱知识点记忆和统一训练计划生成。
```

后端亮点：

```text
封装 Piston 代码执行服务并作为 `CodeExecutionTool` 接入 Agent Workflow，支持 Java 编译运行、测试用例判定和错误日志解析；基于 MyBatis-Plus 接入 MySQL 8，持久化题目、提交、诊断、训练计划和后端知识卡片；当前预留 Redis 配置，热点题目和题目详情缓存待接入；并通过 SSE 实现 Agent 执行步骤流式返回。
```

Agent 亮点：

```text
设计基于状态机的 Agent Workflow，将代码执行、错误分类、代码点评、弱点追踪、训练规划封装为独立 Tool，通过 Agent 编排器串联执行。代码执行是 Tool，测试结果是 Observation，后端维护 Agent 状态和步骤记录，LLM 只负责错误归因和代码点评等语义判断节点。
```
