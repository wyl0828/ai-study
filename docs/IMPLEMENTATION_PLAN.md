...
- Redis 已接入题目列表、题目详情和题目模板热点缓存，失败时降级 MySQL
...
| 缓存 | Redis | 缓存题目列表、题目详情和题目模板；不作为训练数据事实源 |
...
- Dashboard 已通过 `UserController` 接入真实 MySQL 学习数据，并完成教练化重排：统计卡片之后优先展示今日训练项和按天分页的完整训练计划，后续展示薄弱排行、错误类型分布、最近提交、最近模拟面试、合并错题卡、后端知识训练入口和确定性 AI 教练建议；训练计划条目支持完成/跳过，手动重新生成训练计划已暴露接口。单独 hint 查询和独立 accepted-code review REST 端点留到后续阶段。
...
- 题库提交模式已升级为 Hot100 精选 20 题统一 Solution 形态：用户提交非 `public` 的 `class Solution`，后端通过 `CodeWrapper` 注册表按题号包装为 `Main.java` 后送入 Piston。
- 20 题均已补齐面试式题面、3 个测试用例、三层预设提示、solution outline 和完整 Java 参考实现。
...
- 已创建 Spring Boot 后端、MySQL、Redis 题目 / 知识卡只读热点缓存、CORS 配置、基础实体、Mapper 和统一响应结构。
...
- 当前 MVP 种子数据为 Hot100 精选 20 道题、60 条测试用例、1 个 demo 用户。
...
- `UserController` 已暴露 Dashboard 查询接口和学习连续性写接口：统计、薄弱点、弱点事件、错题、最新训练计划、训练计划状态更新、手动重新生成、最近提交、最近模拟面试、错误统计、知识卡自测提交和最近自测记录。
...
- Dashboard 继续只从 `userApi` 的 MySQL-backed endpoints 加载统计、薄弱点、错题卡、错误分布、训练计划、最近提交和最近模拟面试；空数组或 null 展示空状态，不回退 `frontend/lib/mock.ts`。
...
- `frontend/lib/core-loop-stability.node-test.cjs` 覆盖 tab 边界、预设提示边界、SSE fallback 条件、旧流拦截、abort 行为、Dashboard no mock、最近模拟面试、今日优先训练前置、训练计划跳转和空状态文案。
...
MySQL 可以使用本机服务；Redis 用于题目 / 知识卡只读热点缓存且支持 MySQL 降级；Piston 使用 Docker 容器。
...
3. Redis 建议启动用于验证题目缓存；如果 `localhost:6379` 不可用，题目接口会降级 MySQL，核心训练闭环仍可继续。
4. 启动本地 Piston：
...
# Redis 用于题目 / 知识卡热点缓存；不可用时题目和知识训练接口降级 MySQL。
...
- 打开 Dashboard，确认统计、今日优先训练、完整训练计划、弱点排行、错误类型分布、最近提交、最近模拟面试、合并错题卡和 AI 教练建议都来自真实查询接口或现有前端学习数据聚合，不回退 mock。
...
| 题库太多拖慢进度 | MVP 当前稳定 Hot100 精选 20 题，不急于扩到完整 Hot100 |
...
| Solution 模式误判题 | 后端按 `problem.id` 查找 adapter 包装，单测覆盖 20 题 adapter 和 `submit` / `rejudge` 主路径 |
...
11. 讲解后端设计：Piston 封装、Agent Tool、Observation、RAG V1、Memory、SSE、MySQL，以及 Redis 题目列表 / 详情 / 模板、知识卡分类 / 列表 / 详情热点缓存和 MySQL 降级边界。
...
```text
封装 Piston 代码执行服务并作为 `CodeExecutionTool` 接入 Agent Workflow，支持 Java 编译运行、测试用例判定和错误日志解析；基于 MyBatis-Plus 接入 MySQL 8，持久化题目、提交、诊断、训练计划、后端知识卡片和 RAG chunk；Redis 已接入题目列表、题目详情和题目模板热点缓存，缓存不可用时降级到 MySQL；并通过 SSE 实现 Agent 执行步骤流式返回。
```
