...
>
> 说明：后端知识训练为独立 `/knowledge` 模块；RAG V1 已作为 Agent 内部 Tool 接入，基于 MySQL 结构化检索 `problem`、`knowledge_card`、`ai_diagnosis` 和 `mistake_card`，不暴露独立聊天页或公开检索 REST 接口，也不根据算法错因强行推荐 MySQL / Redis / Spring。知识卡内容由 seed profile 显式维护，不在运行时调用 AI 生成。
