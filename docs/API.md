...
> 说明：Phase 2 已实现 Agent 诊断、分层提示、弱点记忆和训练计划的后端工作流，但 Dashboard 查询类接口尚未暴露为 REST Controller。当前前端 Dashboard 使用 mock 数据展示产品形态。
...
当前 `/dashboard` 页面展示以下模块，但数据来自 `frontend/lib/mock.ts`：
