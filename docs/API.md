...
>
> 说明：后端知识训练为独立 `/knowledge` 模块；RAG V1 已作为 Agent 内部 Tool 接入，基于 MySQL 结构化检索 `problem`、`knowledge_card`、`ai_diagnosis` 和 `mistake_card`。当前新增 `/rag-chat` / `POST /api/rag/chat` 作为受控学习资料问答入口，只回答题目、知识卡、历史诊断、错题卡和当前用户学习记录相关问题；它不暴露 raw retrieval，不接入联网搜索，不上传文档，不生成完整 AC 代码，也不根据算法错因强行推荐 MySQL / Redis / Spring。知识卡内容由 seed profile 显式维护，不在运行时调用 AI 生成。
...
受控学习资料问答入口，只回答题目、知识卡、历史诊断、错题卡和当前用户学习记录相关问题。它复用 MySQL RAG V1 检索能力和现有学习记忆数据，不接入通用联网搜索，不上传文档，不生成完整 AC 代码，不替代代码提交诊断主流程。
...
学习记录类问题会直接查询当前 `userId` 的薄弱点、错题卡和最近弱点事件，不依赖 `rag_chunk` 是否已索引。普通题目和知识卡问题继续使用 RAG 检索，并在 Rag Chat 层做二次重排；不会修改 Agent 诊断链路使用的 `RagRetrieveTool`。
...
真实数据由 `scripts/knowledge_card_profiles.cjs` 维护，并通过 `scripts/generate_knowledge_cards_sql.cjs` 生成 `data/knowledge_cards.sql` 和 `frontend/lib/knowledgeSeed.ts`。当前共 120 张结构化知识卡，侧边栏 24 个最终专题每个至少 5 张。内容参考小林 coding 和 JavaGuide 的公开面试知识目录做选题覆盖后重新整理，AI 工程内容按本项目 Agent/RAG 设计原创组织，不复制原文。`/knowledge` 页面本身不是 RAG 查询入口；这些知识卡会被 RAG V1 索引为 Agent 诊断和训练计划的内部证据来源。
