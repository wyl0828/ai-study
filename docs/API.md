...
| `sourceType` | String / null | 推荐来源类型，如 `SUBMISSION_FAILED` / `RAG_KNOWLEDGE_CARD` / `USER_WEAKNESS` / `KNOWLEDGE_CARD_REVIEW` / `MOCK_INTERVIEW_REPORT` |
...
| `statusUpdatedAt` | DateTime / null | 状态更新时间 |
...
趋势按知识卡聚合，同一场面试里同一知识卡的主问题和追问会先求平均分，再与上一场同知识卡面试对比。Dashboard 使用该接口展示“模拟面试趋势”，用于判断复盘后同一知识点是否变稳。
...
会话详情响应包含当前状态、当前题目、turn 列表和报告，前端刷新后通过 `GET /api/mock-interviews/{sessionId}` 恢复，不自行推断状态。前端创建会话后会把地址替换为 `/mock-interview?sessionId={sessionId}`，Dashboard 最近面试入口也使用该参数恢复会话。报告详情在 `/mock-interview?sessionId={sessionId}` 页面内展示：右侧报告区展示平均分、强弱项、推荐知识卡，以及每轮主问题 / 追问、用户回答、命中要点、缺失要点和表达问题，不生成完整 Java AC 代码。
...
前端加载时会并发请求统计、薄弱点、错题、最新训练计划、训练计划历史、最近训练完成情况、最近提交记录、最近模拟面试记录、模拟面试趋势和错误统计。无数据时显示空状态引导文案，不回退 mock 数据。训练计划操作通过 `PATCH /training-plans/items/{itemId}/status` 和 `POST /training-plans/regenerate` 写回后端，再刷新最新计划、历史摘要和最近训练完成情况。上述教练化排序和错题合并属于前端展示层优化；最近模拟面试和模拟面试趋势使用 `UserController` 查询接口返回后端会话摘要与分数变化。
