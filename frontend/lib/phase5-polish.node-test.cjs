...
test("dashboard coach helpers prioritize today's action and grouped mistake review", () => {
  ...
  assert.match(mistakes, /本质问题/);
  assert.match(mistakes, /修复动作/);
  assert.match(mistakes, /复盘口令/);
  assert.match(mistakes, /查看最近 \$\{m\.rawRecords\.length\} 条复盘记录/);
  assert.match(mistakes, /最近复盘记录/);
  assert.doesNotMatch(mistakes, /复盘令/);
  assert.match(mistakes, /xl:grid-cols-3/);
  assert.match(mistakes, /slice\(0,\s*6\)/);
  assert.match(mistakes, /查看全部错题/);
  assert.match(read("lib/dashboardAggregation.ts"), /userFacingErrorTag/);
  assert.match(read("lib/dashboardAggregation.ts"), /deriveMistakePattern/);
